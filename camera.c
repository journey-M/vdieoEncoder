...
void openCamera(void (*fn)(struct picbuffer *), __uint32_t pixfmt){
  ...
  //设置完格式后，要重新查询下，部分内核不支持设置的格式会自动变为其它格式

  struct v4l2_format realFormat;
  ret = ioctl(fd, VIDIOC_G_FMT, &realFormat);

  fprintf(stderr, " ret %d   format : \t%c%c%c%c\n",
          ret,
      		realFormat.fmt.pix.pixelformat & 0xFF,
			(realFormat.fmt.pix.pixelformat >> 8) & 0xFF, 
			(realFormat.fmt.pix.pixelformat >> 16) & 0xFF,
			(realFormat.fmt.pix.pixelformat >> 24) & 0xFF);
